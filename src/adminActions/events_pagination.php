...
for($i = $page * 5 - 5; $i < $page * 5 && $i < sizeof($events); $i++){
    $event = $events[$i];
    ?> <a style="background-color: #1f2342" href="eventDetails?id=<?php echo $event->getId(); ?>" class="list-group-item list-group-item-action flex-column align-items-start mb-3">
        ...
        <p class="text-info">Where: <?php echo htmlspecialchars($event->getLocation()); ?></p>
    </a>
    <?php
} ?>